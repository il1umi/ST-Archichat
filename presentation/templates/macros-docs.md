# ST-Archichat 宏模块使用说明

> 本文档介绍本扩展提供的 `{{roulette}}` 与 `{{cascade}}` 宏。  
> 你可以在「扩展 → 对话规则构筑扩展 → 宏管理」中配置宏组
...
- 权重说明：
  ...
  - 省略权重时默认视为 `1`。
  - 当所有有效权重都为 `0` 或候选为空时，会退化为等概率随机或直接返回原始宏文本，并在控制台输出 `[ST-Archichat][roulette]` 日志。
...
- 配置持久化在 `extensionSettings['st-archichat'].macros.roulette` 下，可通过「导入/导出配置」备份与迁移。
